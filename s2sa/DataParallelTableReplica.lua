--[[
   ...
   To use multiple threads, call DataParallelTableReplica:threads(initFunc).

   For best performance, install NCCL:
    https://github.com/NVIDIA/nccl
    https://github.com/ngimel/nccl.torch
]]--
local DataParallelTableReplica, parent = torch.class('nn.DataParallelTableReplica', 'nn.Container')

local Impls = {}
local BasicImpl = torch.class('nn.DataParallelTableReplica.Basic', Impls)
local ThreadsImpl = torch.class('nn.DataParallelTableReplica.Threads', Impls)
...
function DataParallelTableReplica:threads(initFunc)
   require 'threads'
   self.impl:close()
...
function DataParallelTableReplica:__tostring()
   return 'DataParallelTableReplica: ' .. #self.gpuAssignments .. ' x ' .. tostring(self.modules[1])
end

function DataParallelTableReplica:get(index)
   return self.modules[index]
end

-- this flattens parameters, so that syncParameters and accGradParameters can be much more efficient
function DataParallelTableReplica:flattenParameters()
...
function DataParallelTableReplica:clearState()
   self.impl:exec(function(module)
      module:clearState()
   end)
...
function DataParallelTableReplica:updateOutput(input)
...
function DataParallelTableReplica:moduleParameters()
...
function DataParallelTableReplica:__backward(method, input, gradOutput, scale)
   local prevGpuid = cutorch.getDevice()
...
function DataParallelTableReplica:backward(input, gradOutput, scale)
   return self:__backward('backward', input, gradOutput, scale)
end

function DataParallelTableReplica:updateGradInput(input, gradOutput)
   return self:__backward('updateGradInput', input, gradOutput)
end

function DataParallelTableReplica:accGradParameters(input, gradOutput, scale)
   self:__backward('accGradParameters', input, gradOutput, scale)
end

function DataParallelTableReplica:syncParameters()
   local prevGpuid = cutorch.getDevice()
   if self.flattenedParams and self.usenccl and not cudaLaunchBlocking then
      if #self.gpuAssignments > 1 then
...
function DataParallelTableReplica:accUpdateGradParameters(input, gradOutput, lr)
   error("accUpdateGradParameters not supported for DataParallelTableReplica.")
end

function DataParallelTableReplica:zeroGradParameters()
   local prevGpuid = cutorch.getDevice()
   if self.flattenedParams then
...
function DataParallelTableReplica:updateParameters(learningRate)
   local prevGpuid = cutorch.getDevice()
...
function DataParallelTableReplica:parameters()
   return self.modules[1]:parameters()
end

function DataParallelTableReplica:share(mlp,...)
   error("Share not supported for DataParallelTableReplica")
end

function DataParallelTableReplica:clone(...)
   assert(select('#',...) == 0, "Sharing not supported for DataParallelTableReplica")
   return parent.clone(self)
end

function DataParallelTableReplica:reset(stdv)
   local prevGpuid = cutorch.getDevice()
   cutorch.setDevice(self.gpuAssignments[1])
...
function DataParallelTableReplica:type(typeStr)
   assert(typeStr == 'torch.CudaTensor', 'DataParallelTableReplica supports only torch.CudaTensor type')
...
DataParallelTableReplica.__version = 3

-- DataParallelTableReplica.deserializeNGPUs controls how many GPUs to deserialize
-- upon, otherwise will deserialize to as many GPUs as serialized and error
-- out if it doesn;t have enough available
function DataParallelTableReplica:__read(file, version)
   if version < 2 then
   ...
   -- whether DataParallelTableReplica.deserializeNGPUs is set.
   local gpuAssignments = file:readObject()
   if DataParallelTableReplica.deserializeNGPUs then
      gpuAssignments = {}
      for i = 1, DataParallelTableReplica.deserializeNGPUs do gpuAssignments[i] = i end
      if DataParallelTableReplica.deserializeNGPUs > cutorch.getDeviceCount() then
         error('Deserialization requested on too many GPUs: ' ..
                  DataParallelTableReplica.deserializeNGPUs .. ' vs ' ..
                  cutorch.getDeviceCount() .. ' available')
      end
   end

   -- If DataParallelTableReplica.deserializeNGPUs, deserialization overrides
   ...
   if nGPUs > cutorch.getDeviceCount() then
      error('Model was serialized on ' ..
               ...
               ' please set DataParallelTableReplica.deserializeNGPUs to ignore ' ..
...
function DataParallelTableReplica:__write(file)
...
function DataParallelTableReplica:_reflattenReplicaParameters()
   local flattenedParams = self.flattenedParams
   if flattenedParams then
      self.flattenedParams = self.impl:exec(function(m, i)
         if i == 1 then
...
function DataParallelTableReplica:apply(callback)
...
function DataParallelTableReplica:_broadcast(params)
...
function DataParallelTableReplica:_reduce(gradParams)
...
function DataParallelTableReplica:_distribute(dst, src)
...
function DataParallelTableReplica:_distributeTensorRecursive(dst, src, idx, n)
...
-- Otherwise it does a recursive call on tables.
function DataParallelTableReplica:_concat(dst, src)
...
function DataParallelTableReplica:_concatTensorRecursive(dst, src)
